...
/**
 * Finds all routes from a given folder and binds them to the given router.
 *
 * @param directory The directory that contains routes.
 * @param callback A function that gets called when a file has been found.
 * @return An error if the route could not be loaded.
 */
function exploreRoutes(directory, callback) {
    // Find all the .js files and subdirectories.
    var dirPath = path.join(__dirname, 'routes', directory);

    fs.readdir(dirPath, function(error, routes) {
        // Ignore this directory if an error occured.
        if (error) {
            return error;
        }
        ...
        routes.forEach(function (route) {
            ...
            // If we found a folder, recursively add routes.
            fs.stat(path + '/' + route, function(error, stat) {
                if (error) {
                    return;
                }

                if (stat.isDirectory()) {
                    exploreRoutes(directory + '/' + routePath, router);
                } else if (path.extname(route) == '.js' && callback)  {
                    callback(path + '/' + route);
                }
            });
        });
...
function loadRoutes(app) {
    ...
    exploreRoutes(routePath, function(routePath)
